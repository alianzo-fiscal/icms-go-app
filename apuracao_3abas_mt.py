# coding: utf-8
"""
apuracao_3abas_mt.py
--------------------
Apuracao de ICMS para filiais do Mato Grosso (MT).
Identico ao GO mas:
  - Aliquota interna 20%% (RICMS/MT (Decreto 2.212/2014), vigente desde 01/01/2024)
  - Sem PROTEGE/GO (TO nao possui fundo equivalente)
Uso:
  python apuracao_3abas_mt.py --entradas "Entradas*.xls" --saidas "Saidas*.xls Saidas*.csv"
...
def calc_difal_ent(ent):
    ...
    difal_f  = (grp_base * (ALIQ_INT_MT/100)) - grp_icms
...
def _base_icms_comp(sai):
    ...
    mask = sai['_CFOP'] == '5949'
    df = _agg_sai(sai, mask)
    df['ICMS_COMPLEMENTAR'] = (df['VL_CONTABIL'] * ALIQ_INT_MT / 100).round(2)
    return df
...
def _base_difal_entrada(ent):
    ...
    df['DIFAL_ENTRADA'] = ((df['BASE_ICMS'] * ALIQ_INT_MT / 100) - df['ICMS']).clip(lower=0).round(2)
    return df
...
def gerar_apuracao(output_path, periodo, filiais, debito, credito, difal_e, difal_s, protege):
    ...
    c = ws['A1']
    c.value = 'APURACAO DE ICMS/MT  |  ' + periodo
    ...
    c = ws['A2']
    c.value = 'Valores em R$  |  DIFAL saida: aliq media 17% destino  |  RICMS/MT (Decreto 2.212/2014)'
    ...
    # VI - PROTEGE
    titulo_bloco(row, 'VI.  OBSERVACOES MT  (sem PROTEGE)'); row += 1
    linha(row, 'TO: Aliquota interna 20% desde 01/01/2024 (RICMS/MT (Decreto 2.212/2014) / RICMS/MT art.27,II)', protege.get('p15_por_filial',{}), protege.get('p15_total',0)); row += 1
    ...
    linha(row, 'TOTAL GERAL  ICMS/MT', tot_geral_f, tot_geral, True, cor_geral); row += 1
