# coding: utf-8
"""
apuracao_3abas_df.py
--------------------
Apuracao de ICMS para filiais do Distrito Federal (DF).
Identico ao GO mas:
  - Aliquota interna 20%% (RICMS/DF (Decreto 18.955/97) + Lei 7.326/2023, vigente desde 01/01/2024)
  - Sem PROTEGE/GO (TO nao possui fundo equivalente)
Uso:
  python apuracao_3abas_df.py --entradas "Entradas*.xls" --saidas "Saidas*.xls Saidas*.csv"
...
def calc_difal_ent(ent):
    ...
    difal_f  = (grp_base * (ALIQ_INT_DF/100)) - grp_icms
...
def calc_protege(sai):
    # PROTEGE 15%: base reduzida com aliq. efetiva ~11% (nominal 19% ou 12%)
    # TO: este bloco retorna zeros (sem PROTEGE) mas mantem compatibilidade de interface
    s = sai.copy()
    s['_ALIQ_EF'] = np.where(
        s['_VLCONT'] > 0,
    ...
    # PROTEGE 2%: saidas com aliquota 21%
...
def _base_icms_comp(sai):
    ...
    mask = sai['_CFOP'] == '5949'
    df = _agg_sai(sai, mask)
    df['ICMS_COMPLEMENTAR'] = (df['VL_CONTABIL'] * ALIQ_INT_DF / 100).round(2)
    return df
...
def _base_difal_entrada(ent):
    ...
    # DIFAL = max(0, BASE * 20% - ICMS_PAGO)
    df['DIFAL_ENTRADA'] = ((df['BASE_ICMS'] * ALIQ_INT_DF / 100) - df['ICMS']).clip(lower=0).round(2)
    return df
...
def gerar_apuracao(output_path, periodo, filiais, debito, credito, difal_e, difal_s, protege):
    ...
    c = ws['A1']
    c.value = 'APURACAO DE ICMS/DF  |  ' + periodo
    ...
    c = ws['A2']
    c.value = 'Valores em R$  |  DIFAL saida: aliq media 17% destino  |  RICMS/DF (Decreto 18.955/97) + Lei 7.326/2023'
    ...
    linha(row, 'DIFAL Entrada a Recolher (20% TO - 12%/4% inter.)', difal_e.get('por_filial',{}), difal_e.get('total',0), True); row += 1
    ws.row_dimensions[row].height = 6; row += 1
    ...
    linha(row, 'DIFAL Saida a Recolher (5% = 17% destino - 12% TO)', difal_s.get('por_filial',{}), difal_s.get('total',0), True); row += 1
    ws.row_dimensions[row].height = 6; row += 1
    ...
    # VI - PROTEGE
    titulo_bloco(row, 'VI.  OBSERVACOES DF  (sem PROTEGE)'); row += 1
    linha(row, 'TO: Aliquota interna 20% desde 01/01/2024 (RICMS/DF (Decreto 18.955/97) + Lei 7.326/2023 / RICMS/DF art.27,II)', protege.get('p15_por_filial',{}), protege.get('p15_total',0)); row += 1
    linha(row, 'TO: Nao ha PROTEGE ou fundo equivalente neste estado', protege.get('p2_por_filial',{}), protege.get('p2_total',0)); row += 1
    linha(row, 'TOTAL ENCARGO ADICIONAL TO',
    ...
    linha(row, 'TOTAL GERAL  ICMS/DF', tot_geral_f, tot_geral, True, cor_geral); row += 1
...
def main():
    ...
    output = args.output or str(pasta / ('Apuracao ICMS DF - ' + periodo_nome + '.xlsx'))
    ...
    print('  (Sem PROTEGE/GO — TO nao possui fundo equivalente)', flush=True)
    print('  TOTAL GERAL ICMS TO:         ' + brl(tot_geral), flush=True)
    print('=' * 60, flush=True)
