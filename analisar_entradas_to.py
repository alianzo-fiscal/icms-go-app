"""
analisar_entradas_to.py
Analise de Entradas ICMS/TO (Tocantins) -- gera Excel (.xlsx) e Word (.docx)
Uso: python analisar_entradas_to.py <arquivo1.xls> [arquivo2.csv] ...
"""
...
ALIQ_INT_TO = 20.0               # Lei 4.141/2023 -- vigente desde 01/01/2024
ALIQ_ANT_TO = 18.0               # aliquota anterior (ate 31/12/2023)
DATA_MUDANCA = datetime.date(2024, 1, 1)
...
def calcular_div6(df):
    ...
    if d6.empty:
        return pd.DataFrame(columns=COLS_DIV)
    d6['ICMS Correto (R$)'] = (d6['VLBASEICMS'] * ALIQ_INT_TO / 100).round(2)
    d6['Dif. ICMS (R$)'] = (d6['VLICMS'] - d6['ICMS Correto (R$)']).round(2)
    d6['Critica'] = 'Aliquota 18% em NF apos 01/01/2024 -- aliq. geral TO e 20% (Lei 4.141/2023)'
...
def _aba_resumo(wb, df, divs, periodo):
    ...
    COR = '1F4E79'
    _escrever_titulo(ws, f'Analise de Entradas ICMS/TO -- {periodo}', COR, 4)
...
def _aba_sintese(wb, divs, periodo):
    ...
    COR = '1F4E79'
    _escrever_titulo(ws, f'Sintese de Riscos -- Entradas ICMS/TO -- {periodo}', COR, 7)
    ...
    grupos_info = {
        ...
        'DIV6': {'grupo': 'Aliq. 18% apos 01/01/2024 (TO)', 'natureza': 'Aliquota defasada -- Lei 4.141/2023 alterou para 20%', 'risco': 'ATENCAO', 'cor': 'ED7D31'},
    }
    r = 5
...
def gerar_word(df, divs_dict, periodo, caminho_word):
    ...
    h = doc.add_heading(f'Analise de Entradas ICMS/TO -- {periodo}', 0)
    ...
    infos = [
        ('DIV-1', 'Mercadoria Importada com Aliquota > 4%', 'CRITICA',
         'Mercadorias de origem estrangeira (Orig 1, 2 ou 6) tributadas com aliquota superior a 4% em operacoes interestaduais violam a Resolucao do Senado 13/2012. O credito escriturado a maior pode ser glosado pela SEFAZ-TO.',
        ...
        ('DIV-4', 'Base Reduzida sem CST 20', 'ATENCAO',
         'Base de calculo inferior ao valor do item com CST 00 declarado e inconsistente. O EFD exige CST 20 quando ha reducao de base por convenio CONFAZ. Confirmar o convenio aplicavel na SEFAZ-TO (dtri.sefaz.to.gov.br).',
        ...
        ('DIV-5', 'Aliquota Atipica Interestadual', 'VERIFICAR',
         ...
         'Verificar o estado de origem do fornecedor; confirmar se ha convenio ou regime especial; corrigir o CFOP se o fornecedor for tocantinense.'),
        ('DIV-6', 'Aliquota 18% apos 01/01/2024 (Especifico TO)', 'ATENCAO',
         'A aliquota geral interna do Tocantins foi alterada de 18% para 20% em 01/01/2024 pela Lei 4.141/2023 (CTE-TO, art. 27, II). NFs intraestaduais com CST 00 emitidas apos essa data ainda a 18% indicam que o ERP do fornecedor nao foi atualizado.',
         'Notificar os fornecedores tocantinenses para regularizar a emissao a 20%; o credito do destinatario nao e afetado (o erro e do emitente), mas sinaliza fornecedor com ERP desatualizado.'),
    ]
...
def main():
    if len(sys.argv) < 2:
        print('Uso: python analisar_entradas_to.py <arquivo1.xls> [arquivo2.csv] ...')
        sys.exit(1)
