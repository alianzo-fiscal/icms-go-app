...
# Configurações PVA — usa Path.home() para funcionar em qualquer máquina
# Todos do time precisam ter o projeto PVA na mesma pasta relativa ao perfil Windows
_HOME = Path.home()
_PVA_BASE     = _HOME / "Claude" / "Projects" / "Transformar Apuracao em Arquivo SPED ICMS" / "pva_monitor"
...
with tab_sped:
    ...
    if not IS_LOCAL:
        ...
        with st.expander("⚙️ Caminhos configurados", expanded=False):
            ...
            st.caption(
                "Para alterar, adicione em `st.secrets` a seção `[pva]` com as chaves: "
                "`pasta_monitorada`, `log_validacao`, `fase1_bat`, `fase2_bat`."
            )
        ...
        # ── Seção 2: Fase 1 — Validação em Lote ───────────────────────────────
        ...
        if not fase1_bat.exists():
            st.caption(f"⚠️ `fase1_lote.bat` não encontrado em: `{fase1_bat}`")

        if btn_fase1:
            with st.spinner(
                f"Validando {len(txt_na_pasta)} arquivo(s) no PVA... "
                "Não use o computador durante este processo."
            ):
                try:
                    result = subprocess.run(
                        ["cmd", "/c", str(fase1_bat)],
                    ...
                    if result.returncode == 0:
                        ...
                        st.warning(f"⚠️ Fase 1 terminou com código {result.returncode}. Verifique os resultados abaixo.")
                    if result.stdout.strip():
                        with st.expander("📋 Log da Fase 1", expanded=False):
                            st.code(result.stdout, language="text")
                    if result.stderr.strip():
                        with st.expander("⚠️ Erros da Fase 1", expanded=False):
                            st.code(result.stderr, language="text")
                    st.rerun()
                except subprocess.TimeoutExpired:
                    st.error("❌ Tempo limite excedido (20 min). Verifique o PVA manualmente.")
                except Exception:
                    st.error(f"❌ Erro ao executar Fase 1:\n\n```\n{traceback.format_exc()}\n```")

        st.divider()

        # ── Seção 3: Resultados da Validação ──────────────────────────────────
        st.markdown("### 3. Resultados da Validação")

        col_r1, col_r2 = st.columns([3, 1])
        with col_r2:
            if st.button("🔄 Atualizar", key="btn_atualizar_log"):
                st.rerun()

        resultados = _ler_resultado_validacao()

        if not resultados:
            st.info("ℹ️ Nenhum resultado de validação encontrado. Execute a Fase 1 primeiro.")
        else:
            ok_count    = sum(1 for r in resultados if r.get("status") == "OK")
            erro_count  = sum(1 for r in resultados if r.get("status") != "OK")
            fase2_count = sum(1 for r in resultados if r.get("fase2_ok"))

            c1, c2, c3 = st.columns(3)
            c1.metric("✅ Validados (OK)", ok_count)
            c2.metric("❌ Com Erro", erro_count)
            c3.metric("📤 Transmitidos (Fase 2)", fase2_count)

            st.markdown("#### Detalhamento por arquivo")
            for r in resultados:
                status = r.get("status", "?")
                arquivo = r.get("arquivo", "—")
                ts = r.get("timestamp", "")
                fase2_ok = r.get("fase2_ok", False)
                fase2_ts = r.get("fase2_timestamp", "")

                icon = "✅" if status == "OK" else "❌"
                f2_badge = " 📤 Transmitido" if fase2_ok else ""
                with st.expander(f"{icon} {arquivo}{f2_badge}", expanded=False):
                    st.markdown(f"**Status validação:** `{status}`  |  **Data/hora:** {ts}")
                    if fase2_ok:
                        st.markdown(f"**Fase 2 (transmissão):** ✅ concluída em {fase2_ts}")
                    else:
                        st.markdown("**Fase 2:** aguardando")

        st.divider()

        # ── Seção 4: Fase 2 — Assinar e Transmitir ────────────────────────────
        st.markdown("### 4. Fase 2 — Assinar e Transmitir ao SEFAZ")

        pendentes_fase2 = [
            r for r in resultados
            if r.get("status") == "OK" and not r.get("fase2_ok")
        ]

        if pendentes_fase2:
            st.success(
                f"📋 {len(pendentes_fase2)} arquivo(s) validado(s) e prontos para transmissão:\n\n"
                + "\n".join(f"  • {r['arquivo']}" for r in pendentes_fase2)
            )
        else:
            st.info("ℹ️ Nenhum arquivo pendente de transmissão.")

        st.warning(
            "⚠️ **Pré-requisito:** O certificado digital (e-CNPJ) deve estar configurado no PVA "
            "em *Configurações → Certificado Digital* antes de executar a Fase 2. "
            "Sem isso, a assinatura falhará."
        )

        btn_fase2 = st.button(
            "📤 Iniciar Fase 2 (Assinar + Transmitir)",
            type="primary",
            key="btn_fase2",
            disabled=not (pendentes_fase2 and fase2_bat.exists()),
        )

        if not fase2_bat.exists():
            st.caption(f"⚠️ `fase2_assinar_transmitir.bat` não encontrado em: `{fase2_bat}`")

        if btn_fase2:
            with st.spinner("Executando Fase 2 — Assinatura e Transmissão... Não use o computador."):
                try:
                    result = subprocess.run(
                        ["cmd", "/c", str(fase2_bat)],
                        input="SIM\n",
                    ...
                    if result.returncode == 0:
                        st.success("✅ Fase 2 concluída! Verifique os recibos no PVA.")
                    else:
                        st.warning(f"⚠️ Fase 2 terminou com código {result.returncode}.")
                    if result.stdout.strip():
                        with st.expander("📋 Log da Fase 2", expanded=False):
                            st.code(result.stdout, language="text")
                    if result.stderr.strip():
                        with st.expander("⚠️ Erros da Fase 2", expanded=False):
                            st.code(result.stderr, language="text")
                    st.rerun()
                except subprocess.TimeoutExpired:
                    st.error("❌ Tempo limite excedido (30 min). Verifique o PVA manualmente.")
                except Exception:
                    st.error(f"❌ Erro ao executar Fase 2:\n\n```\n{traceback.format_exc()}\n```")

# ── Rodapé ────────────────────────────────────────────────────────────────────
st.markdown("---")
st.caption(
    "Alianzo Consultoria Fiscal · ICMS/GO · "
