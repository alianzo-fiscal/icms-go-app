#!/usr/bin/env python3
"""
analisar_saidas_pa.py
Analise de Saidas ICMS/PA (Para) -- gera Excel (.xlsx) e Word (.docx)
Uso: python analisar_saidas_pa.py <arquivo1.csv> [arquivo2.xls] ...
"""
...
ALIQ_INT_PA   = 19.0          # RICMS/PA (Decreto 4.676/2001) -- aliq. geral PA
...
def calcular_div1(df):
    ...
    if d1.empty:
        return d1
    d1['ICMS Correto (R$)'] = (d1['VLBASEICMS'] * ALIQ_INT_PA / 100).round(2)
    d1['Dif. ICMS (R$)'] = (d1['VLICMS'] - d1['ICMS Correto (R$)']).round(2)
    d1['Critica'] = d1.apply(
        lambda r: f'Aliq {r["PERCICMS"]}% em NCM {r["NCM"]} -- MT nao prevê 21%. Aliq geral TO e 20% (RICMS/PA (Decreto 2.212/2014)). Verificar supérfluo (25%) ou padrao (20%).'
    )
    return d1
...
def calcular_div2(intra):
    ...
    if d2.empty:
        return d2
    d2['ICMS Correto (R$)'] = (d2['VLBASEICMS'] * ALIQ_INT_PA / 100).round(2)
...
def calcular_div4(intra):
    ...
    d4['Critica'] = d4['reducao_pct'].apply(
        lambda r: f'CST 20 com reducao de {r:.1f}% -- confirmar convenio CONFAZ/PA e percentual correto.'
    )
...
def gerar_excel(df, divs, grp, periodo, caminho_excel):
    ...
    for sname in wb.sheetnames:
        ...
        t = ws['A1']
        if sname == 'RESUMO':
            t.value = f'ANALISE DE SAIDAS ICMS/PA -- {periodo.upper()}'
        else:
            t.value = f'{sname} -- Divergencias de Saidas ICMS/PA -- {periodo}'
        t.font = WHITE_FONT
        t.fill = DARK_FILL
...
def gerar_word(df, divs_dict, periodo, caminho_word):
    ...
    doc = Document()
    h = doc.add_heading(f'Analise de Saidas ICMS/PA -- {periodo}', 0)
    ...
    infos_div = [
        ('DIV-1', 'Aliquota 21% -- Atipica em TO', 'CRITICA',
         'A aliquota de 21% nao esta prevista no RICMS/PA vigente. A aliquota geral do Para e de 20% desde 01/01/2024 (RICMS/PA (Decreto 2.212/2014)). Risco bidirecional: se deveria ser 20%, ha recolhimento a maior (recuperavel); se supérfluo (25%), ha recolhimento a menor.',
         'Verificar a tabela de aliquotas do RICMS/PA para cada NCM; se padrao, corrigir para 20%; se supérfluo, verificar se 25% se aplica no TO.'),
        ('DIV-2', 'Aliquota 12% Intraestadual com CST 00', 'ATENCAO',
         'Aliquota de 12% em operacao interna sem CST 20 significa que a reducao de base nao esta formalmente declarada no EFD. Verificar se ha convenio CONFAZ ou Lei TO especifica para o NCM. A SEFAZ-PA (www.sefa.pa.gov.br) disponibiliza a legislacao vigente.',
        ...
        ('DIV-7', 'CST 40/41 -- Verificar Convenio', 'VERIFICAR',
         'Isencoes e nao tributacoes devem ter amparo em convenio CONFAZ ou lei estadual especifica. Produtos como livros tem imunidade constitucional; demais precisam de verificacao individual. Consultar www.sefa.pa.gov.br para a lista de beneficios vigentes no TO.',
...
def main():
    if len(sys.argv) < 2:
        print('Uso: python analisar_saidas_pa.py <arquivo1.csv> [arquivo2.xls] ...')
        sys.exit(1)
