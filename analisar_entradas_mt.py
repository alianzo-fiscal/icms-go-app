"""
analisar_entradas_mt.py
Analise de Entradas ICMS/MT (Mato Grosso) -- gera Excel (.xlsx) e Word (.docx)
Uso: python analisar_entradas_mt.py <arquivo1.xls> [arquivo2.csv] ...
"""
...
def _aba_resumo(wb, df, divs, periodo):
    ...
    COR = '1F4E79'
    _escrever_titulo(ws, f'Analise de Entradas ICMS/MT -- {periodo}', COR, 4)
...
def _aba_sintese(wb, divs, periodo):
    ...
    COR = '1F4E79'
    _escrever_titulo(ws, f'Sintese de Riscos -- Entradas ICMS/MT -- {periodo}', COR, 7)
...
def gerar_word(df, divs_dict, periodo, caminho_word):
    ...
    h = doc.add_heading(f'Analise de Entradas ICMS/MT -- {periodo}', 0)
    ...
    infos = [
        ('DIV-1', 'Mercadoria Importada com Aliquota > 4%', 'CRITICA',
         'Mercadorias de origem estrangeira (Orig 1, 2 ou 6) tributadas com aliquota superior a 4% em operacoes interestaduais violam a Resolucao do Senado 13/2012. O credito escriturado a maior pode ser glosado pela SEFAZ-MT.',
        ...
        ('DIV-4', 'Base Reduzida sem CST 20', 'ATENCAO',
         'Base de calculo inferior ao valor do item com CST 00 declarado e inconsistente. O EFD exige CST 20 quando ha reducao de base por convenio CONFAZ. Confirmar o convenio aplicavel na SEFAZ-MT (www.sefaz.mt.gov.br).',
        ...
        ('DIV-5', 'Aliquota Atipica Interestadual', 'VERIFICAR',
         ...
         'Verificar o estado de origem do fornecedor; confirmar se ha convenio ou regime especial; corrigir o CFOP se o fornecedor for mato-grossense.'),
    ]
...
def main():
    if len(sys.argv) < 2:
        print('Uso: python analisar_entradas_mt.py <arquivo1.xls> [arquivo2.csv] ...')
        sys.exit(1)
