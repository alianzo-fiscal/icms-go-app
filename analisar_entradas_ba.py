"""
analisar_entradas_ba.py
Analise de Entradas ICMS/BA (Bahia) -- gera Excel (.xlsx) e Word (.docx)
Uso: python analisar_entradas_ba.py <arquivo1.xls> [arquivo2.csv] ...
"""
...
ALIQ_INT_BA = 20.5               # RICMS/BA (Decreto 6.284/97) + Lei 14.629/2023 -- vigente desde 07/02/2024
...
def calcular_div6(df):
    """DIV-6: Entradas intraestaduais (CFOP 1xxx) com aliquota 18% apos 01/01/2024 (TO especifico)"""
    intra = df[df['TIPO_OP'] == 'Intraestadual'].copy()
    ...
    if d6.empty:
        return pd.DataFrame(columns=COLS_DIV)
    d6['ICMS Correto (R$)'] = (d6['VLBASEICMS'] * ALIQ_INT_BA / 100).round(2)
    d6['Dif. ICMS (R$)'] = (d6['VLICMS'] - d6['ICMS Correto (R$)']).round(2)
    d6['Critica'] = 'Aliquota 19.0% em NF apos 07/02/2024 -- aliq. geral BA e 20.5% (RICMS/BA (Decreto 6.284/97) + Lei 14.629/2023)'
    return d6[_selecionar_cols(d6)]
...
def _aba_resumo(wb, df, divs, periodo):
    ...
    COR = '1F4E79'
    _escrever_titulo(ws, f'Analise de Entradas ICMS/BA -- {periodo}', COR, 4)
    ...
    infos_resumo = [
        ...
        ('DIV3', 'ATENCAO'), ('DIV4', 'ATENCAO'), ('DIV5', 'VERIFICAR'), ('DIV6', 'ATENCAO'),
    ]
...
def _aba_sintese(wb, divs, periodo):
    ...
    COR = '1F4E79'
    _escrever_titulo(ws, f'Sintese de Riscos -- Entradas ICMS/BA -- {periodo}', COR, 7)
    ...
    grupos_info = {
        ...
        'DIV6': {'grupo': 'Aliq. 18% apos 01/01/2024 (TO)', 'natureza': 'Aliquota defasada -- RICMS/BA (Decreto 6.284/97) + Lei 14.629/2023 alterou para 20%', 'risco': 'ATENCAO', 'cor': 'ED7D31'},
    }
    r = 5
...
def gerar_excel(df, divs, periodo, caminho_excel):
    ...
    abas_div = [
        ...
        ('DIV6', 'ED7D31', 'DIV-6 -- Aliq. 18% apos 01/01/2024 -- TO (ATENCAO)'),
    ]
...
def gerar_word(df, divs_dict, periodo, caminho_word):
    ...
    h = doc.add_heading(f'Analise de Entradas ICMS/BA -- {periodo}', 0)
    ...
    infos = [
        ('DIV-1', 'Mercadoria Importada com Aliquota > 4%', 'CRITICA',
         'Mercadorias de origem estrangeira (Orig 1, 2 ou 6) tributadas com aliquota superior a 4% em operacoes interestaduais violam a Resolucao do Senado 13/2012. O credito escriturado a maior pode ser glosado pela SEFAZ-BA.',
        ...
        ('DIV-4', 'Base Reduzida sem CST 20', 'ATENCAO',
         'Base de calculo inferior ao valor do item com CST 00 declarado e inconsistente. O EFD exige CST 20 quando ha reducao de base por convenio CONFAZ. Confirmar o convenio aplicavel na SEFAZ-BA (www.sefaz.ba.gov.br).',
        ...
        ('DIV-5', 'Aliquota Atipica Interestadual', 'VERIFICAR',
         ...
         'Verificar o estado de origem do fornecedor; confirmar se ha convenio ou regime especial; corrigir o CFOP se o fornecedor for baense.'),
        ('DIV-6', 'Aliquota 18% apos 01/01/2024 (Especifico TO)', 'ATENCAO',
         'A aliquota geral interna do Bahia foi alterada de 18% para 20% em 01/01/2024 pela RICMS/BA (Decreto 6.284/97) + Lei 14.629/2023. NFs intraestaduais com CST 00 emitidas apos essa data ainda a 18% indicam que o ERP do fornecedor nao foi atualizado.',
         'Notificar os fornecedores baenses para regularizar a emissao a 20.5%; o credito do destinatario nao e afetado (o erro e do emitente), mas sinaliza fornecedor com ERP desatualizado.'),
    ]
...
def main():
    if len(sys.argv) < 2:
        print('Uso: python analisar_entradas_ba.py <arquivo1.xls> [arquivo2.csv] ...')
        sys.exit(1)
    ...
    divs = {'DIV1': d1, 'DIV2': d2, 'DIV3': d3, 'DIV4': d4, 'DIV5': d5, 'DIV6': d6}
    for k, v in divs.items():
        n = len(v) if v is not None and not v.empty else 0
        print(f'  {k}: {n} registros')

    caminho_excel = PASTA_SAIDA / f'Analise Entradas ICMS BA - {periodo}.xlsx'
    caminho_word  = PASTA_SAIDA / f'Analise Entradas ICMS BA - {periodo}.docx'
