...
Análise de Entradas ICMS/GO — gera Excel (.xlsx) e Word (.docx)
...
COLS_DIV = [
    ...
    'PERCICMS', 'VLICMS', 'ICMS Correto (R$)', 'Dif. ICMS (R$)', 'Crítica', '_arquivo'
]

CFOP_DESC = {
    '2923': 'complemento de preço ou bonificação financeira',
    '2557': 'frete CIF — integra base ICMS da mercadoria',
    '2910': 'bonificação/brinde — CST correto: 40 ou 41',
    '2551': 'ativo imobilizado — verificar se é frete ou instalação',
    '2556': 'retorno de serviço',
    '2949': 'materiais gráficos/amostras',
}
...
def _selecionar_cols(df):
    """Retorna apenas as colunas COLS_DIV que existem no df."""
...
def calcular_div2(inter):
    ...
    d2['Dif. ICMS (R$)'] = 0.0
    d2['Crítica'] = 'CST 41 interestadual — solicitar documentação do convênio/isenção aplicada'
...
def calcular_div3(inter):
    ...
    d3['Dif. ICMS (R$)'] = 0.0
    d3['Crítica'] = d3['CODFISCAL'].apply(
        lambda c: f'CFOP {c} + CST 90: {CFOP_DESC.get(c, "verificar natureza da operação")}'
    )
...
def calcular_div4(inter):
    ...
    d4['Crítica'] = d4['reducao_pct'].apply(
        lambda r: f'Base reduzida ~{r:.1f}% com CST 00 — EFD exige CST 20 para base reduzida por convênio'
    )
...
def calcular_div5(inter):
    ...
    d5['Dif. ICMS (R$)'] = 0.0
    d5['Crítica'] = d5['PERCICMS'].apply(
        lambda a: f'Alíquota {a}% não prevista na tabela interestadual (esperado: 7% ou 12%)'
    )
...
def _colorir_dif(ws, col_idx, n_linhas, linha_dados=5):
    """Colorir coluna 'Dif. ICMS (R$)' — verde se > 0, vermelho se < 0."""
...
def _escrever_aba_div(wb, nome_aba, df_div, cor_hex, titulo):
    """Escreve uma aba de divergência."""
    ws = wb.create_sheet(nome_aba)

    if df_div is None or df_div.empty:
        _escrever_titulo(ws, titulo, cor_hex, 5)
        ws.cell(row=3, column=1, value='Nenhuma divergência encontrada nesta categoria.')
    ...
    for i, (_, row) in enumerate(df_out.iterrows(), start=5):
        for j, col in enumerate(colunas, 1):
            ...
            c.border = _thin_border()
            c.alignment = Alignment(vertical='center', wrap_text=(col == 'Crítica'))
    ...
    for j, col in enumerate(colunas, 1):
        ...
        if col in ('DESCRICAO', 'Crítica', 'PARTICIPANTE'):
            ws.column_dimensions[ltr].width = 45
        elif col in ('VLITEM', 'VLBASEICMS', 'VLICMS', 'ICMS Correto (R$)', 'Dif. ICMS (R$)'):
            ws.column_dimensions[ltr].width = 18
        elif col in ('DATA', 'NUMNOTA', 'CODFISCAL', 'CST', 'ORIGMERCTRIB', 'NCM', 'PERCICMS'):
...
def _aba_resumo(wb, df, divs, periodo):
    ...
    COR = '1F4E79'
    _escrever_titulo(ws, f'Análise de Entradas ICMS/GO — {periodo}', COR, 4)
    ...
    c = ws.cell(row=r, column=1, value='VISÃO GERAL')
    ...
    for row_data in dados:
        ...
        for j, val in enumerate(row_data, 1):
            ...
            cell.border = _thin_border() if val else None

    # Tabela divergências
    r += 2
    ws.merge_cells(f'A{r}:D{r}')
    c = ws.cell(row=r, column=1, value='RESUMO DE DIVERGÊNCIAS')
    c.font = Font(bold=True, color='FFFFFF', size=11)
    c.fill = PatternFill('solid', fgColor=COR)
    c.alignment = Alignment(horizontal='center')
    ...
    cores_crit = {'CRÍTICA': 'C00000', 'ATENÇÃO': 'ED7D31', 'VERIFICAR': '375623'}
    ...
    infos_resumo = [
        ('DIV1', 'CRÍTICA'), ('DIV2', 'ATENÇÃO'),
        ('DIV3', 'ATENÇÃO'), ('DIV4', 'ATENÇÃO'), ('DIV5', 'VERIFICAR'),
    ]
    for div_id, crit in infos_resumo:
        ...
        dif = (
            round(dados_div['Dif. ICMS (R$)'].sum(), 2)
            if dados_div is not None and not dados_div.empty and 'Dif. ICMS (R$)' in dados_div.columns
            else 0.0
        )
        ...
        for j, val in enumerate(vals, 1):
            ...
            if j == 2:
                c.font = Font(bold=True, color=cores_crit.get(crit, '000000'))
            if j == 4:
                c.number_format = '#,##0.00'
...
def _aba_sintese(wb, divs, periodo):
    ...
    COR = '1F4E79'
    _escrever_titulo(ws, f'Síntese de Riscos — Entradas ICMS/GO — {periodo}', COR, 7)

    colunas = [
        'DIV', 'GRUPO', 'VALOR ENVOLVIDO (R$)', 'REGISTROS',
        'IMP. ICMS (R$)', 'NATUREZA PROVÁVEL', 'RISCO'
    ]
    _escrever_cabecalho(ws, colunas, COR, linha=4)

    grupos_info = {
        'DIV1': {
            'grupo': 'Importada Alíq. > 4%',
            'natureza': 'Crédito indevido — Res. Senado 13/2012',
            'risco': 'CRÍTICA',
            'cor': 'C00000',
        },
        'DIV2': {
            'grupo': 'CST 41 Interestadual',
            'natureza': 'Isenção sem amparo ou convênio não documentado',
            'risco': 'ATENÇÃO',
            'cor': 'ED7D31',
        },
        'DIV3': {
            'grupo': 'CST 90 Interestadual',
            'natureza': 'Natureza da operação não identificada',
            'risco': 'ATENÇÃO',
            'cor': 'ED7D31',
        },
        'DIV4': {
            'grupo': 'Base Reduzida sem CST 20',
            'natureza': 'CST incorreto — EFD exige CST 20',
            'risco': 'ATENÇÃO',
            'cor': 'ED7D31',
        },
        'DIV5': {
            'grupo': 'Alíquota Atípica Inter.',
            'natureza': 'Erro de cadastro ou convênio não documentado',
            'risco': 'VERIFICAR',
            'cor': '375623',
        },
    }
    ...
    for div_id, info in grupos_info.items():
        ...
        for j, val in enumerate(vals, 1):
            ...
            if j in (3, 5):
                c.number_format = '#,##0.00'
            if j == 7:
                c.font = Font(bold=True, color=info['cor'])
        r += 1

    for ltr, w in [('A', 10), ('B', 28), ('C', 22), ('D', 12), ('E', 18), ('F', 45), ('G', 12)]:
        ws.column_dimensions[ltr].width = w
...
def gerar_excel(df, divs, periodo, caminho_excel):
    ...
    abas_div = [
        ('DIV1', 'C00000', 'DIV-1 — Importada com Alíquota > 4% (CRÍTICA)'),
        ('DIV2', 'ED7D31', 'DIV-2 — CST 41 Interestadual (ATENÇÃO)'),
        ('DIV3', 'ED7D31', 'DIV-3 — CST 90 Interestadual (ATENÇÃO)'),
        ('DIV4', 'ED7D31', 'DIV-4 — Base Reduzida sem CST 20 (ATENÇÃO)'),
        ('DIV5', '375623', 'DIV-5 — Alíquota Atípica Interestadual (VERIFICAR)'),
    ]
...
def gerar_word(df, divs_dict, periodo, caminho_word):
    ...
    h = doc.add_heading(f'Análise de Entradas ICMS/GO — {periodo}', 0)
    ...
    infos = [
        (
            'DIV-1', 'Mercadoria Importada com Alíquota > 4%', 'CRÍTICA',
            'Mercadorias de origem estrangeira (Orig 1, 2 ou 6) tributadas com alíquota superior a 4% em '
            'operações interestaduais violam a Resolução do Senado 13/2012. O crédito escriturado a maior '
            'pode ser glosado pela SEFAZ-GO.',
            'Notificar os fornecedores para emissão de nota de correção; estornar o crédito indevido no EFD; '
            'verificar se há necessidade de complemento de DIFAL.',
        ),
        (
            'DIV-2', 'CST 41 Interestadual — Verificar Isenção', 'ATENÇÃO',
            'CST 41 em operações interestaduais indica isenção ou não tributação. Sem amparo em convênio '
            'CONFAZ específico, o crédito pode ser considerado indevido. A coexistência de CST 41 e CST 00 '
            'no mesmo NCM/fornecedor é o cenário de maior risco.',
            'Solicitar ao fornecedor o convênio ou ato normativo que ampara a isenção; verificar se o mesmo '
            'produto aparece com CST 00 em outras NFs; arquivar documentação de amparo.',
        ),
        (
            'DIV-3', 'CST 90 Interestadual — Natureza Desconhecida', 'ATENÇÃO',
            'CST 90 em operação interestadual indica tributação "outras", o que pode encobrir diferentes '
            'naturezas de operação. O CFOP é a chave diagnóstica: CFOP 2923 com valores altos frequentemente '
            'representa complemento de preço ou bonificação financeira.',
            'Mapear a natureza de cada operação pelo CFOP; contatar fornecedor para esclarecimento; corrigir '
            'o CST conforme a natureza identificada.',
        ),
        (
            'DIV-4', 'Base Reduzida sem CST 20', 'ATENÇÃO',
            'Base de cálculo inferior ao valor do item com CST 00 declarado é tecnicamente inconsistente. '
            'O EFD exige CST 20 quando há redução de base por convênio CONFAZ. Ratios fixos (0,9390 ou '
            '0,9112) indicam parametrização sistêmica do ERP.',
            'Identificar o convênio de amparo para cada cluster de redução; corrigir a parametrização do ERP '
            'para emitir CST 20; retificar o EFD dos períodos afetados.',
        ),
        (
            'DIV-5', 'Alíquota Atípica Interestadual', 'VERIFICAR',
            'Alíquotas fora do padrão interestadual (0%, 4%, 7% ou 12%) podem indicar erro de cadastro no '
            'ERP do fornecedor, convênio especial não documentado, ou fornecedor local incorretamente '
            'classificado com CFOP 2xxx.',
            'Verificar o estado de origem do fornecedor; confirmar se há convênio ou regime especial; '
            'corrigir o CFOP se o fornecedor for goiano.',
        ),
    ]
    ...
    for idx, (div_id, titulo, crit, narrativa, acao) in enumerate(infos, 2):
        ...
        p = doc.add_paragraph()
        clr = (
            RGBColor(0xC0, 0x00, 0x00) if crit == 'CRÍTICA'
            else RGBColor(0xC5, 0x5A, 0x11) if crit == 'ATENÇÃO'
            else RGBColor(0x37, 0x56, 0x23)
        )
        r = p.add_run(f'  {crit} — {n} registros identificados')
        r.bold = True
        ...
        p2.add_run('Ação recomendada: ').bold = True
        p2.add_run(acao)
        doc.add_paragraph()
...
def main():
    if len(sys.argv) < 2:
        print('Uso: python analisar_entradas.py <arquivo1.xls> [arquivo2.csv] ...')
        sys.exit(1)
