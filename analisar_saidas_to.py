#!/usr/bin/env python3
"""
analisar_saidas_to.py
Analise de Saidas ICMS/TO (Tocantins) -- gera Excel (.xlsx) e Word (.docx)
Uso: python analisar_saidas_to.py <arquivo1.csv> [arquivo2.xls] ...
"""
...
ALIQ_INT_TO   = 20.0          # Lei 4.141/2023 -- desde 01/01/2024
...
def calcular_div1(df):
    ...
    if d1.empty:
        return d1
    d1['ICMS Correto (R$)'] = (d1['VLBASEICMS'] * ALIQ_INT_TO / 100).round(2)
    d1['Dif. ICMS (R$)'] = (d1['VLICMS'] - d1['ICMS Correto (R$)']).round(2)
    d1['Critica'] = d1['NCM'].apply(
        lambda n: f'Aliq 21% em NCM {n} -- TO nao prevê 21%. Aliq geral TO e 20% (Lei 4.141/2023). Verificar supérfluo (25%) ou padrao (20%).'
    )
    return d1
...
def calcular_div2(intra):
    ...
    if d2.empty:
        return d2
    d2['ICMS Correto (R$)'] = (d2['VLBASEICMS'] * ALIQ_INT_TO / 100).round(2)
...
def calcular_div4(intra):
    ...
    d4['Critica'] = d4['reducao_pct'].apply(
        lambda r: f'CST 20 com reducao de {r:.1f}% -- confirmar convenio CONFAZ/TO e percentual correto.'
    )
...
def calcular_div8(intra):
    ...
    if d8.empty:
        return pd.DataFrame(columns=COLS_DIV)
    d8['ICMS Correto (R$)'] = (d8['VLBASEICMS'] * ALIQ_INT_TO / 100).round(2)
    d8['Dif. ICMS (R$)'] = (d8['VLICMS'] - d8['ICMS Correto (R$)']).round(2)
    d8['Critica'] = 'Saida intraestadual a 18% apos 01/01/2024 -- aliq. geral TO e 20% (Lei 4.141/2023). ICMS recolhido a menor.'
...
def gerar_excel(df, divs, grp, periodo, caminho_excel):
    ...
    for sname in wb.sheetnames:
        ...
        t = ws['A1']
        if sname == 'RESUMO':
            t.value = f'ANALISE DE SAIDAS ICMS/TO -- {periodo.upper()}'
        else:
            t.value = f'{sname} -- Divergencias de Saidas ICMS/TO -- {periodo}'
        t.font = WHITE_FONT
        t.fill = DARK_FILL
...
def gerar_word(df, divs_dict, periodo, caminho_word):
    ...
    doc = Document()
    h = doc.add_heading(f'Analise de Saidas ICMS/TO -- {periodo}', 0)
    ...
    infos_div = [
        ('DIV-1', 'Aliquota 21% -- Atipica em TO', 'CRITICA',
         'A aliquota de 21% nao esta prevista no RCTE-TO vigente. A aliquota geral do Tocantins e de 20% desde 01/01/2024 (Lei 4.141/2023). Risco bidirecional: se deveria ser 20%, ha recolhimento a maior (recuperavel); se supérfluo (25%), ha recolhimento a menor.',
         'Verificar a tabela de aliquotas do RCTE-TO para cada NCM; se padrao, corrigir para 20%; se supérfluo, verificar se 25% se aplica no TO.'),
        ('DIV-2', 'Aliquota 12% Intraestadual com CST 00', 'ATENCAO',
         'Aliquota de 12% em operacao interna sem CST 20 significa que a reducao de base nao esta formalmente declarada no EFD. Verificar se ha convenio CONFAZ ou Lei TO especifica para o NCM. A SEFAZ-TO (dtri.sefaz.to.gov.br) disponibiliza a legislacao vigente.',
        ...
        ('DIV-7', 'CST 40/41 -- Verificar Convenio', 'VERIFICAR',
         'Isencoes e nao tributacoes devem ter amparo em convenio CONFAZ ou lei estadual especifica. Produtos como livros tem imunidade constitucional; demais precisam de verificacao individual. Consultar dtri.sefaz.to.gov.br para a lista de beneficios vigentes no TO.',
        ...
        ('DIV-8', 'Saida Intraestadual a 18% apos 01/01/2024 (Especifico TO)', 'ATENCAO',
         'A aliquota geral interna do Tocantins foi alterada de 18% para 20% em 01/01/2024 pela Lei 4.141/2023 (CTE-TO, art. 27, II). Saidas intraestaduais emitidas apos essa data ainda a 18% indicam ERP com aliquota desatualizada, gerando ICMS recolhido a menor e risco de autuacao na SEFAZ-TO.',
...
def main():
    if len(sys.argv) < 2:
        print('Uso: python analisar_saidas_to.py <arquivo1.csv> [arquivo2.xls] ...')
        sys.exit(1)
