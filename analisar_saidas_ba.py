#!/usr/bin/env python3
"""
analisar_saidas_ba.py
Analise de Saidas ICMS/BA (Bahia) -- gera Excel (.xlsx) e Word (.docx)
Uso: python analisar_saidas_ba.py <arquivo1.csv> [arquivo2.xls] ...
"""
...
ALIQ_INT_BA   = 20.5          # RICMS/BA (Decreto 6.284/97) + Lei 14.629/2023 -- desde 07/02/2024
ALIQ_ANT_BA   = 19.0          # aliquota anterior
DATA_MUDANCA  = datetime.date(2024, 2, 7)
...
def calcular_div1(df):
    ...
    d1 = df[df['PERCICMS'].isin([19.0, 20.0, 21.0])].copy()
    if d1.empty:
        return d1
    d1['ICMS Correto (R$)'] = (d1['VLBASEICMS'] * ALIQ_INT_BA / 100).round(2)
    d1['Dif. ICMS (R$)'] = (d1['VLICMS'] - d1['ICMS Correto (R$)']).round(2)
    d1['Critica'] = d1.apply(
        lambda r: f'Aliq {r["PERCICMS"]}% em NCM {r["NCM"]} -- BA nao prevê 21%. Aliq geral TO e 20% (RICMS/BA (Decreto 6.284/97) + Lei 14.629/2023). Verificar supérfluo (25%) ou padrao (20%).'
    )
    return d1
...
def calcular_div2(intra):
    ...
    if d2.empty:
        return d2
    d2['ICMS Correto (R$)'] = (d2['VLBASEICMS'] * ALIQ_INT_BA / 100).round(2)
...
def calcular_div4(intra):
    ...
    d4['Critica'] = d4['reducao_pct'].apply(
        lambda r: f'CST 20 com reducao de {r:.1f}% -- confirmar convenio CONFAZ/BA e percentual correto.'
    )
...
def calcular_div8(intra):
    ...
    if d8.empty:
        return pd.DataFrame(columns=COLS_DIV)
    d8['ICMS Correto (R$)'] = (d8['VLBASEICMS'] * ALIQ_INT_BA / 100).round(2)
    d8['Dif. ICMS (R$)'] = (d8['VLICMS'] - d8['ICMS Correto (R$)']).round(2)
    d8['Critica'] = 'Saida intraestadual a 18% apos 01/01/2024 -- aliq. geral TO e 20% (RICMS/BA (Decreto 6.284/97) + Lei 14.629/2023). ICMS recolhido a menor.'
...
def gerar_excel(df, divs, grp, periodo, caminho_excel):
    ...
    for sname in wb.sheetnames:
        ...
        t = ws['A1']
        if sname == 'RESUMO':
            t.value = f'ANALISE DE SAIDAS ICMS/BA -- {periodo.upper()}'
        else:
            t.value = f'{sname} -- Divergencias de Saidas ICMS/BA -- {periodo}'
        t.font = WHITE_FONT
        t.fill = DARK_FILL
...
def gerar_word(df, divs_dict, periodo, caminho_word):
    ...
    doc = Document()
    h = doc.add_heading(f'Analise de Saidas ICMS/BA -- {periodo}', 0)
    ...
    infos_div = [
        ('DIV-1', 'Aliquota 21% -- Atipica em TO', 'CRITICA',
         'A aliquota de 21% nao esta prevista no RICMS/BA vigente. A aliquota geral do Bahia e de 20% desde 01/01/2024 (RICMS/BA (Decreto 6.284/97) + Lei 14.629/2023). Risco bidirecional: se deveria ser 20%, ha recolhimento a maior (recuperavel); se supérfluo (25%), ha recolhimento a menor.',
         'Verificar a tabela de aliquotas do RICMS/BA para cada NCM; se padrao, corrigir para 20%; se supérfluo, verificar se 25% se aplica no TO.'),
        ('DIV-2', 'Aliquota 12% Intraestadual com CST 00', 'ATENCAO',
         'Aliquota de 12% em operacao interna sem CST 20 significa que a reducao de base nao esta formalmente declarada no EFD. Verificar se ha convenio CONFAZ ou Lei TO especifica para o NCM. A SEFAZ-BA (www.sefaz.ba.gov.br) disponibiliza a legislacao vigente.',
        ...
        ('DIV-7', 'CST 40/41 -- Verificar Convenio', 'VERIFICAR',
         'Isencoes e nao tributacoes devem ter amparo em convenio CONFAZ ou lei estadual especifica. Produtos como livros tem imunidade constitucional; demais precisam de verificacao individual. Consultar www.sefaz.ba.gov.br para a lista de beneficios vigentes no TO.',
        ...
        ('DIV-8', 'Saida Intraestadual a 18% apos 01/01/2024 (Especifico TO)', 'ATENCAO',
         'A aliquota geral interna do Bahia foi alterada de 18% para 20% em 01/01/2024 pela RICMS/BA (Decreto 6.284/97) + Lei 14.629/2023. Saidas intraestaduais emitidas apos essa data ainda a 18% indicam ERP com aliquota desatualizada, gerando ICMS recolhido a menor e risco de autuacao na SEFAZ-BA.',
...
def main():
    if len(sys.argv) < 2:
        print('Uso: python analisar_saidas_ba.py <arquivo1.csv> [arquivo2.xls] ...')
        sys.exit(1)
    ...
    caminho_excel = PASTA_SAIDA / f'Analise Saidas ICMS BA - {periodo}.xlsx'
    caminho_word  = PASTA_SAIDA / f'Analise Saidas ICMS BA - {periodo}.docx'
